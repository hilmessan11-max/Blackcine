...
@section('title', 'Modifier Programme d\'Affiliation - BlackCine')
...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mt-2">Modifier le programme : {{ $program->name }}</h1>
    </div>

    <div class="bg-white rounded-lg shadow overflow-hidden max-w-2xl mx-auto">
        <form action="{{ route('admin.advertising.affiliate.update', $program) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="affiliate_id" id="affiliate_id" value="{{ old('affiliate_id', $program->affiliate_id) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>

                <div>
                    <label for="base_url" class="block text-sm font-medium text-gray-700 mb-1">URL de base du programme</label>
                    <input type="url" name="base_url" id="base_url" value="{{ old('base_url', $program->base_url) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>

                <div>
                    <label for="commission_rate" class="block text-sm font-medium text-gray-700 mb-1">Taux de commission estimé (%)</label>
                    <input type="number" name="commission_rate" id="commission_rate" value="{{ old('commission_rate', $program->commission_rate) }}" min="0" max="100" step="0.1"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $program->description) }}</textarea>
                </div>
