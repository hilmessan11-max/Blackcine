...
@section('title', 'Nouvelle Notification Push - BlackCine')
...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mt-2">Créer une notification push</h1>
    </div>

    <div class="bg-white rounded-lg shadow overflow-hidden max-w-4xl mx-auto">
        <form action="{{ route('admin.notifications.push.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Ex: Nouveau film disponible !"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>

                <div>
                    <label for="message" class="block text-sm font-medium text-gray-700 mb-1">Message *</label>
                    <textarea name="message" id="message" rows="4" required placeholder="Découvrez dès maintenant notre nouveau film exclusif..."
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="url" name="image_url" id="image_url" value="{{ old('image_url') }}" placeholder="https://..."
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>

                    <div>
                        <label for="target_url" class="block text-sm font-medium text-gray-700 mb-1">URL de destination (Optionnelle)</label>
                        <input type="url" name="target_url" id="target_url" value="{{ old('target_url') }}" placeholder="https://blackcine.com/film/123"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="target_audience" id="target_audience" required
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="all" {{ old('target_audience') == 'all' ? 'selected' : '' }}>Tous les utilisateurs</option>
                        <option value="subscribers" {{ old('target_audience') == 'subscribers' ? 'selected' : '' }}>Abonnés Premium uniquement</option>
                        <option value="free_users" {{ old('target_audience') == 'free_users' ? 'selected' : '' }}>Utilisateurs gratuits uniquement</option>
                        <option value="specific_users" {{ old('target_audience') == 'specific_users' ? 'selected' : '' }}>Utilisateurs spécifiques</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="scheduled_at" id="scheduled_at" value="{{ old('scheduled_at') }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <p class="text-xs text-gray-500 mt-1">Laissez vide pour enregistrer comme brouillon sans planification.</p>
                </div>
