...
@section('title', 'Modifier l\'article - BlackCine Admin')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-3xl font-bold text-gray-800">Modifier l'article</h1>
        <p class="text-gray-600 mt-2">Édition de : {{ $article->title }}</p>
    </div>
    <div class="flex space-x-3">
        ...
        <a href="#" target="_blank" class="text-blue-600 hover:text-blue-800 flex items-center px-3 py-2 rounded-lg hover:bg-blue-50 transition-colors">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" id="excerpt" rows="3"
                            class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50 transition-colors shadow-sm">{{ old('excerpt', $article->excerpt) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="body" id="body" rows="15" required
                            class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50 transition-colors shadow-sm font-mono text-sm">{{ old('body', $article->body) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="meta_title" value="{{ old('meta_title', $article->meta_title ?? '') }}" class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50 transition-colors shadow-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Méta-description</label>
                        <textarea name="meta_description" rows="2" class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50 transition-colors shadow-sm">{{ old('meta_description', $article->meta_description ?? '') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="published_at" value="{{ old('published_at', $article->created_at->format('Y-m-d')) }}"
                            class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50 transition-colors shadow-sm">
                    </div>
                ...
                <div class="space-y-4">
                    @if($article->image)
                        <div class="relative rounded-lg overflow-hidden mb-3 group">
                            <img src="{{ asset('storage/' . $article->image) }}" alt="Image actuelle" class="w-full h-auto object-cover">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="categories[]" id="categories" multiple size="4"
                            ...
                            <option value="{{ $category->id }}" {{ $article->categories->contains($category->id) ? 'selected' : '' }} class="p-1">
                                {{ $category->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="tags[]" id="tags" multiple size="4"
                            class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50 transition-colors shadow-sm text-sm">
                            @foreach($tags as $tag)
                            <option value="{{ $tag->id }}" {{ $article->tags->contains($tag->id) ? 'selected' : '' }} class="p-1">
                                {{ $tag->name }}
                            </option>
                            @endforeach
                        </select>
