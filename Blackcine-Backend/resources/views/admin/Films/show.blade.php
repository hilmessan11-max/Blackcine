...
@section('title', $film->title . ' - Gestion Films')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-3xl font-bold text-gray-800">{{ $film->title }}</h1>
        <p class="text-gray-600 mt-2">{{ $film->overview }}</p>
    </div>
    <div class="flex space-x-3">
        <a href="{{ route('admin.films.edit', $film->id) }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
            Modifier
        </a>
        <a href="{{ route('admin.films.index') }}" class="bg-gray-300 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-400">
            Retour
        </a>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <span class="ml-2 text-gray-900">{{ $film->category }}</span>
                </div>
                @if($film->year)
                <div>
                    <span class="text-sm font-medium text-gray-600">Année :</span>
                    <span class="ml-2 text-gray-900">{{ $film->year }}</span>
                </div>
                @endif
                @if($film->country)
                <div>
                    <span class="text-sm font-medium text-gray-600">Pays :</span>
                    <span class="ml-2 text-gray-900">{{ $film->country }}</span>
                </div>
                @endif
                @if($film->language)
                <div>
                    <span class="text-sm font-medium text-gray-600">Langue :</span>
                    <span class="ml-2 text-gray-900">{{ $film->language }}</span>
                </div>
                @endif
            </div>
        </div>

        @if($film->genres && count($film->genres) > 0)
        <div class="bg-white rounded-lg shadow p-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-2">
                <a href="{{ route('admin.films.edit', $film->id) }}" class="block w-full text-center bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                    Modifier
                </a>
                <form action="{{ route('admin.films.destroy', $film->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce film ?');" class="w-full">
                    @csrf
                    @method('DELETE')
